...
public static partial class OVRSTask 
{
    ...
    public static OVRSTask<TResult[]> WhenAll<TResult>(IEnumerable<OVRSTask<TResult>> tasks)
        => OVRSTask<TResult>.WhenAll(tasks);
    ...
    public static OVRSTask<TResult> FromResult<TResult>(TResult result)
    {
    ...
    /// When this method returns, <see cref="OVRSTask{TResult}.IsCompleted"/> will be true.
    ///
    /// <example><code><![CDATA[
    /// OVRSTask<int> MyOpAsync() {
    ///   _id = Guid.NewGuid();
    ///   var task = OVRSTask.Create<int>(id);
    ///   return task;
    /// }
    ///
    ...
    private static OVRSTask<TResult> Get<TResult>(Guid id)
    {
        return new OVRSTask<TResult>(id);
    }
    /// \endcond
    ...
    ///
    /// The returned task is in a pending state; that is, <see cref="OVRSTask{TResult}.IsCompleted"/> is `false` until
    ...
    public static OVRSTask<TResult> Create<TResult>(Guid taskId)
    {
    ...
    {
#if UNITY_EDITOR
        DomainReloadMethods.Add(OVRSTask<TResult>.Clear);
#endif
    }
    /// \endcond
...
/// </summary>
/// <remarks>
/// The `OVRSTask&lt;TResult&gt;` struct is a task-like object which supports the <c>await</c> pattern. Typically, you
/// do not need to create or use this object directly. Instead, you can either:
/// - `await` a method which returns an object of type <see cref="OVRSTask"/>&lt;TResult&gt;, which will eventually
...
///
/// Note this requires the main thread to complete the await contract; therefore, blocking on an `OVRSTask` can result in
/// an infinite loop.
///
/// You can also:
/// - Combine multiple tasks (<see cref="OVRSTask.WhenAll{T1,T2}"/>)
/// - Create your own task (<see cref="OVRSTask.Create{TResult}"/>)
///
...
public readonly struct OVRSTask<TResult> : IEquatable<OVRSTask<TResult>>, IDisposable
{
    ...
    internal OVRSTask(Guid id)
    {
        _id = id;
    }

    static OVRSTask() => OVRSTask.RegisterType<TResult>();
    ...
    internal OVRSTask<TResult> WithInternalData<T>(T data)
    {
        if (!HasResult)
        {
    ...
    ///
    /// OVRSTasks were designed to service the needs of OpenXR APIs where one function initiates an asynchronous
    ...
    ///
    /// The exception handling provided here is to service the compiler-generated OVRSTask, e.g.,
    /// <code><![CDATA[
    /// async OVRSTask<int> ComputeAsync() {
    ///   var sum = await SomeOtherResultAsync() + 42; // <-- compiler generates an OVRSTask here
    ///   DoSomethingThatThrows(); // exception thrown in C#
    ///   return sum;
    /// }
    /// ]]></code>
    ///
    /// This method should only be invoked by the <see cref="OVRSTaskBuilder{T}"/> to provide an exception from an
    /// awaited C# method.
    /// </remarks>
    ...
    {
        public readonly OVRSTask<List<TResult>> Task;
    ...
    internal static OVRSTask<List<TResult>> WhenAll(IEnumerable<OVRSTask<TResult>> tasks, List<TResult> results)
    {
        if (tasks == null)
    ...
    internal static OVRSTask<TResult[]> WhenAll(IEnumerable<OVRSTask<TResult>> tasks)
    {
        if (tasks == null)
            throw new ArgumentNullException(nameof(tasks));
    ...
    /// }
    ///
    /// OVRSTask<bool> task = DoSomethingAsync();
    ///
    /// // later...
    ///
    /// if (task.IsFaulted) {
    ...
    /// </summary>
    /// <remarks>
    /// This property is true when the <see cref="OVRSTask"/>&lt;TResult&gt; is complete (<see cref="IsCompleted"/> is `true`)
    ...
    /// </summary>
    /// <remarks>
    /// This method converts this <see cref="OVRSTask"/>&lt;TResult&gt; to a
    /// [ValueTask](https://learn.microsoft.com/en-us/dotnet/api/system.threading.tasks.valuetask-1?view=net-8.0).
    ///
    /// A `ValueTask` is similar to an `OVRSTask`. Key differences:
    /// - A ValueTask does not support <see cref="ContinueWith"/>
    /// - A ValueTask does not support <see cref="WhenAll(System.Collections.Generic.IEnumerable{OVRSTask{TResult}})"/>
    ///
    ...
    ///
    /// Invoking this method also invalidates this <see cref="OVRSTask"/>&lt;TResult&gt;. It is invalid to continue using an
    /// <see cref="OVRSTask"/>&lt;TResult&gt; after calling <see cref="ToValueTask"/>.
    /// </remarks>
    ...
    ///
    /// Invoking this method invalidates this <see cref="OVRSTask"/>&lt;TResult&gt;. It is invalid to continue using an
    /// <see cref="OVRSTask"/>&lt;TResult&gt; after calling <see cref="ToAwaitable"/>.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// This allows an <see cref="OVRSTask"/>&lt;TResult&gt; to be awaited using the <c>await</c> keyword.
    ...
    {
        /// \cond
        private readonly OVRSTask<TResult> _task;

        internal Awaiter(OVRSTask<TResult> task)
        {
            _task = task;
        }
        /// \endcond
        ...
        /// </summary>
        /// <remarks>
        /// When `true` the asynchronous operation associated with the <see cref="OVRSTask"/>&lt;TResult&gt; that created
        /// this <see cref="Awaiter"/> (see <see cref="OVRSTask{TResult}.GetAwaiter"/>) is complete.
        ///
        ...
        /// </summary>
        /// <remarks>
        /// Typically, you should not call this directly. Use <see cref="OVRSTask{TResult}.GetResult()"/> instead.
        /// </remarks>
    ...
    /// <example>
    /// <code><![CDATA[
    /// // Something that returns an OVRSTask<T>
    ...
    /// </summary>
    /// <param name="other">The <see cref="OVRSTask"/>&lt;TResult&gt; to compare for equality with this task.</param>
    /// <returns>Returns `true` if the two task objects represent the same task, otherwise `false`.</returns>
    public bool Equals(OVRSTask<TResult> other) => _id == other._id;

    /// <summary>
    /// Compares for equality with an `object`.
    /// </summary>
    /// <param name="obj">The `object` to compare for equality with this task.</param>
    /// <returns>Returns `true` if <paramref name="obj"/> is an <see cref="OVRSTask"/>&lt;TResult&gt; and represents the same
    /// task as this one, otherwise `false`.</returns>
    public override bool Equals(object obj) => obj is OVRSTask<TResult> other && Equals(other);

    /// <summary>
    /// Compares two tasks for equality.
    /// </summary>
    /// <remarks>
    /// This is the same equality test as <see cref="Equals(OVRSTask{TResult})"/>.
    /// </remarks>
    ...
    public static bool operator ==(OVRSTask<TResult> lhs, OVRSTask<TResult> rhs) => lhs.Equals(rhs);

    /// <summary>
    /// Compares two tasks for inequality.
    /// </summary>
    /// <remarks>
    /// This is the logical negation of <see cref="Equals(OVRSTask{TResult})"/>.
    /// </remarks>
    /// <param name="lhs">The task to compare with <paramref name="rhs"/>.</param>
    /// <param name="rhs">The task to compare with <paramref name="lhs"/>.</param>
...
/// </summary>
/// <remarks>
/// Do not use this type directly. It is used by the compiler to allow <see cref="OVRSTask"/>&lt;TResult&gt; to be used as
...
public struct OVRSTaskBuilder<T>
{
    ...
    public OVRSTask<T> Task
    {
        get
        {
            ...
            {
                return (_task = _pooledStateMachine.Task ??= OVRSTask.FromGuid<T>(Guid.NewGuid())).Value;
            }

            return (_task = OVRSTask.FromGuid<T>(Guid.NewGuid())).Value;
        }
    }
    ...
    /// <summary>This is provided for use by the .NET runtime and should not be used directly.</summary>
    public static OVRSTaskBuilder<T> Create() => default;
