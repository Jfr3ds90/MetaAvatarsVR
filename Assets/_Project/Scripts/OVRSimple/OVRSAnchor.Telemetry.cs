...
partial struct OVRSAnchor
{
    ...
    static class Telemetry
    {
        ...
        // to map (markerId, requestId) pairs to a particular OVRSTelemetryMarker
        private readonly struct Key : IEquatable<Key>
        {
            ...
            public Key(OVRSTelemetryMarker marker, ulong requestId) => (_markerId, _requestId) = (marker.MarkerId, requestId);
        ...
        private static Dictionary<Key, OVRSTelemetryMarker> s_markers = new();

        // Called from OVRSAnchor.Init
        public static void OnInit() => s_markers.Clear();

        public static void AddMarker(ulong requestId, OVRSTelemetryMarker marker)
            => s_markers.Add(new(marker, requestId), marker);

        public static OVRSTelemetryMarker Start(MarkerId markerId, ulong requestId, OVRPlugin.Result result)
        {
            var marker = OVRSTelemetry.Start((int)markerId);
        ...
        public static void SetSyncResult(OVRSTelemetryMarker marker, ulong requestId, OVRPlugin.Result result)
        {
        ...
        public static OVRSTelemetryMarker? SetAsyncResult(MarkerId markerId, ulong requestId, long result)
        ...
        public static OVRSTelemetryMarker? GetMarker(MarkerId markerId, ulong requestId)
            => TryGetMarker(markerId, requestId, out var marker) ? marker : null;

        public static bool TryGetMarker(MarkerId markerId, ulong requestId, out OVRSTelemetryMarker marker)
            => s_markers.TryGetValue(new(markerId, requestId), out marker);

        public static bool Remove(MarkerId markerId, ulong requestId, out OVRSTelemetryMarker marker)
            => s_markers.Remove(new(markerId, requestId), out marker);

        public static OVRSTelemetryMarker? GetRemove(MarkerId markerId, ulong requestId)
