...
// e.g., an array.
struct OVRSNativeList<T> : IDisposable, IReadOnlyList<T> where T : unmanaged
{
    ...
    public OVRSNativeList(int? initialCapacity, Allocator allocator)
    {
    ...
    public OVRSNativeList(Allocator allocator)
    {
        _array = default;
        _allocator = allocator;
        Count = 0;
    ...
    public static unsafe implicit operator T*(OVRSNativeList<T> list) => list.Data;

    public static implicit operator Span<T>(OVRSNativeList<T> list) => list.AsSpan();

    public static implicit operator ReadOnlySpan<T>(OVRSNativeList<T> list) => list.AsReadOnlySpan();
...
internal static class OVRSNativeList
{
    ...
    public readonly struct CapacityHelper
    {
        ...
        public OVRSNativeList<T> AllocateEmpty<T>(Allocator allocator) where T : unmanaged => new(_count, allocator);
    }
    ...
    // }
    public static CapacityHelper WithSuggestedCapacityFrom<T>([NoEnumeration] IEnumerable<T> collection,
        out OVRSEnumerable<T> nonAllocatingEnumerable)
    {
        nonAllocatingEnumerable = collection.ToNonAlloc();
        return new CapacityHelper(nonAllocatingEnumerable.Count);
    }

    // Creates a new OVRSNativeList and copies the elements from collection into it.
    // The caller owns the memory and must dispose it.
    public static OVRSNativeList<T> ToNativeList<T>(this IEnumerable<T> collection, Allocator allocator)
        where T : unmanaged
    {
        var list = new OVRSNativeList<T>(allocator);
