...
/// </summary>
/// <remarks>
/// This component can be accessed from an <see cref="OVRSAnchor"/> that supports it by calling
/// <see cref="OVRSAnchor.GetComponent{T}"/> from the anchor.
///
...
public readonly partial struct OVRSLocatable : IOVRSAnchorComponent<OVRSLocatable>, IEquatable<OVRSLocatable>
{
    ...
    /// </summary>
    /// <remarks>
    /// Obtain a <see cref="TrackingSpacePose"/> from <see cref="OVRSLocatable.TryGetSceneAnchorPose"/> or
    /// <see cref="OVRSLocatable.TryGetSpatialAnchorPose"/> depending on the type of anchor you wish to interpret it as.
    ///
    ...
    /// </summary>
    /// <remarks>
    /// This is a jobified version of <see cref="OVRSLocatable.TryGetSceneAnchorPose"/>. The
    /// <see cref="TrackingSpacePose"/> for each <see cref="OVRSLocatable"/> in <see cref="Locatables"/> is determined
    /// and written to <see cref="Poses"/>.
    ///
    /// The length of <see cref="Locatables"/> and <see cref="Poses"/> must be equal.
    ///
    /// An element of <see cref="Locatables"/> may be invalid (that is, <see cref="OVRSLocatable.IsNull"/> is `true`),
    /// in which case the <see cref="TrackingSpacePose.Position"/> and <see cref="TrackingSpacePose.Rotation"/>
    /// properties corresponding to this <see cref="OVRSLocatable"/> will be `null`.
    ///
    ...
    public struct GetSceneAnchorPosesJob : IJobFor
    {
        ...
        /// </remarks>
        [ReadOnly]
        public NativeArray<OVRSLocatable> Locatables;
    ...
    /// </summary>
    /// <remarks>
    /// This is a jobified version of <see cref="OVRSLocatable.TryGetSpatialAnchorPose"/>. The
    /// <see cref="TrackingSpacePose"/> for each <see cref="OVRSLocatable"/> in <see cref="Locatables"/> is determined
    /// and written to <see cref="Poses"/>.
    ///
    /// The length of <see cref="Locatables"/> and <see cref="Poses"/> must be equal.
    ...
    static JobHandle ScheduleUpdateTransforms(
        NativeArray<OVRSLocatable> locatables,
    ...
    /// </summary>
    /// <remarks>
    /// This method accepts a collection of <see cref="OVRSAnchor"/>-`Transform` pairs and attempts to get each anchor's
    ...
    ///
    ///     Dictionary<OVRSAnchor, Transform> _anchors = new();
    ///
    ///     void Update()
    ///     {
    ///         OVRSLocatable.UpdateSceneAnchorTransforms(_anchors, _cameraRig.transform, null);
    ///     }
    /// }
    /// ]]></code>
    /// </example>
    ///
    /// This method is similar to calling <see cref="TryGetSceneAnchorPose"/> on each <see cref="OVRSAnchor"/> in
    ...
    /// </remarks>
    /// <param name="anchors">A collection of anchor-transform pairs. The pose of each <see cref="OVRSAnchor"/> is used
    ...
    /// <see cref="TrackingSpacePose"/> of each <see cref="OVRSAnchor"/> is added to it. This is an optional parameter,
    ...
    public static void UpdateSceneAnchorTransforms(IEnumerable<KeyValuePair<OVRSAnchor, Transform>> anchors,
        ...
        List<TrackingSpacePose> trackingSpacePoses = null)
    {
        ...
        using var locatables = OVRSNativeList
            .WithSuggestedCapacityFrom(anchors)
            .AllocateEmpty<OVRSLocatable>(Allocator.TempJob);

        using var transformAccessArray = new TransformAccessArray(capacity: locatables.Capacity);

        static OVRSLocatable GetLocatableOrDefault(OVRSAnchor anchor) =>
            anchor.TryGetComponent<OVRSLocatable>(out var locatable) ? locatable : default;

        if (anchors is Dictionary<OVRSAnchor, Transform> dict)
        {
