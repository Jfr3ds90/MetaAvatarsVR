...
/// - <see cref="OVRSpatialAnchor.ShareAsync(OVRSSpaceUser)"/>
/// - <see cref="OVRSpatialAnchor.ShareAsync(OVRSSpaceUser,OVRSSpaceUser)"/>
/// - <see cref="OVRSpatialAnchor.ShareAsync(OVRSSpaceUser,OVRSSpaceUser,OVRSSpaceUser)"/>
/// - <see cref="OVRSpatialAnchor.ShareAsync(OVRSSpaceUser,OVRSSpaceUser,OVRSSpaceUser,OVRSSpaceUser)"/>
/// - <see cref="OVRSpatialAnchor.ShareAsync(IEnumerable{OVRSSpaceUser})"/>
/// - <see cref="OVRSpatialAnchor.ShareAsync(IEnumerable{OVRSpatialAnchor},IEnumerable{OVRSSpaceUser})"/>
///
/// An <see cref="OVRSSpaceUser"/> is a lightweight struct that wraps a native handle to a space user. You can create
/// an <see cref="OVRSSpaceUser"/> from the user id of an Oculus user obtained from the
/// [Platform SDK](https://developer.oculus.com/documentation/unity/ps-platform-intro/).
/// </remarks>
public struct OVRSSpaceUser : System.IDisposable
{
    /// <summary>
    ...
    /// </remarks>
    public static bool TryCreate(ulong platformUserId, out OVRSSpaceUser spaceUser)
    {
        spaceUser = new OVRSSpaceUser();
    ...
    /// This is a common value to inadvertently obtain, usually in cases involving insufficient app, user, or device permissions.
    /// </remarks>
    public static bool TryCreate(string platformUserId, out OVRSSpaceUser spaceUser)
    {
    ...
    /// </summary>
    /// <remarks>
    /// The <see cref="OVRSSpaceUser"/> may be invalid if it has been disposed (<see cref="Dispose"/>) or if creation
    /// fails.
    /// <example>
    /// For example:
    /// <code><![CDATA[
    ...
    /// or use the <see cref="TryCreate(ulong,out OVRSSpaceUser)"/> static method instead.
    /// </remarks>
    [System.Obsolete("Constructor ignores validation. Use TryCreate(*) methods instead.", error: false)]
    public OVRSSpaceUser(ulong spaceUserId)
    {
        _ = OVRPlugin.CreateSpaceUser(spaceUserId, out _handle);
    }

    /// <summary>
    /// The org-unique Oculus user ID associated with this <see cref="OVRSSpaceUser"/>.
    /// </summary>
    /// <remarks>
    /// This property is the `spaceUserId` argument provided to the  <see cref="OVRSSpaceUser"/> constructor, or zero
    /// if the <see cref="OVRSSpaceUser"/> is not valid (<see cref="Valid"/> is false).
    /// </remarks>
    public ulong Id => _handle == 0 ? 0 : OVRPlugin.GetSpaceUserId(_handle, out var userId) ? userId : 0;

    /// <summary>
    /// Disposes of the <see cref="OVRSSpaceUser"/>.
    /// </summary>
    /// <remarks>
