...
partial struct OVRSAnchor
{
    ...
    internal static OVRSTask<bool> CreateDeferredSpaceComponentStatusTask(ulong space, SpaceComponentType componentType, bool enabledDesired, double timeout
    )
    {
        ...
        {
            list = OVRSObjectPool.List<DeferredValue>();
            _deferredTasks.Add(key, list);
        }

        var task = OVRSTask.FromGuid<bool>(Guid.NewGuid());
