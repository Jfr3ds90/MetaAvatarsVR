...
namespace MetaAvatarsVR.Networking
{
    ...
    public class NetworkedSpotLight : NetworkBehaviour
    {
        ...
        // IMPORTANTE: Usar el mismo callback que las demás propiedades para evitar race conditions
        [Networked, OnChangedRender(nameof(OnLightPropertiesChanged))]
        ...
        {
           Debug.Log($"[NetworkedSpotLight] {name} - Spawned. HasAuthority: {HasStateAuthority}");
        ...
        private void InitializeNetworkedProperties()
        {
           ...
           Debug.Log($"[NetworkedSpotLight] {name} - Initialized network properties");
           Debug.Log($"  Enabled: {LightEnabled}, Color: {LightColor}");
           Debug.Log($"  Intensity: {LightIntensity}, Range: {LightRange}");
           Debug.Log($"  RenderLayer: {RenderingLayerMask}");
        }
        ...
        private void SyncLightProperties()
        {
            ...
            if (hasChanges)
            {
               Debug.Log($"[NetworkedSpotLight] {name} - Properties synchronized");
            }
        }
        ...
        public void UpdateLightSettings(bool enabled, Color color, float intensity, uint renderLayer)
        {
           ...
           Debug.Log($"[NetworkedSpotLight] {name} - Settings updated manually");
           Debug.Log($"  Enabled: {enabled}, Color: {color}");
           Debug.Log($"  Intensity: {intensity}, RenderLayer: {renderLayer}");
        }
        #endregion
        ...
        private void OnLightEnabledChanged()
        {
            if (spotLight != null && !HasStateAuthority)
            {
               ...
               Debug.Log($"[NetworkedSpotLight] {name} - Light enabled changed to: {LightEnabled}");
            }
        }
        ...
        private void OnLightPropertiesChanged()
        {
            if (!HasStateAuthority)
            {
                ...
                // Log detallado cuando cambian propiedades críticas
               Debug.Log($"[NetworkedSpotLight] {name} - Properties changed on remote client");
               Debug.Log($"  Color: {LightColor}, Intensity: {LightIntensity}");
               Debug.Log($"  RenderLayer Mask: {RenderingLayerMask} (Binary: {Convert.ToString(RenderingLayerMask, 2).PadLeft(32, '0')})");
            }
        }
        ...
        private void ApplyNetworkedProperties()
        {
            ...
            int targetMask = (int)RenderingLayerMask;
            spotLight.renderingLayerMask = targetMask;

            // Verificar que se aplicó correctamente
           Debug.Log($"[NetworkedSpotLight] {name} - Properties applied (HasAuthority: {HasStateAuthority})");
           Debug.Log($"  Enabled: {LightEnabled}, Color: {LightColor}, Intensity: {LightIntensity}");
           Debug.Log($"  Networked RenderLayer Mask: {RenderingLayerMask} (Binary: {Convert.ToString(RenderingLayerMask, 2).PadLeft(32, '0')})");
           Debug.Log($"  Applied to Light.renderingLayerMask: {spotLight.renderingLayerMask} (Binary: {Convert.ToString((uint)spotLight.renderingLayerMask, 2).PadLeft(32, '0')})");
        }
        #endregion
        ...
        private void ValidateSetup()
        {
            if (spotLight == null)
            {
               Debug.LogError($"[NetworkedSpotLight] {name} - Light component required!");
            }
            else if (spotLight.type != LightType.Spot)
            {
               Debug.LogWarning($"[NetworkedSpotLight] {name} - Light is not a Spot light. Type: {spotLight.type}");
            }

            var networkObject = GetComponent<NetworkObject>();
            if (networkObject == null)
            {
               Debug.LogError($"[NetworkedSpotLight] {name} - NetworkObject component required!");
            }

            if (parentFlashlight == null)
            {
               Debug.LogWarning($"[NetworkedSpotLight] {name} - No parent NetworkedFlashlight found. This component works best as a child of NetworkedFlashlight.");
            }
        }
        #endregion
